...
class DockerManager:
    ...
    def docker_compose_up(self, compose_file_path):
        """Executa o comando 'podman-compose up' com tratamento robusto de ambiente."""
        ...
        try:
            ...
            # Obtém o diretório do arquivo compose
            ...
            if self.container_engine == "podman":
                try:
                    # Usa o caminho absoluto para evitar problemas de path
                    cmd = ["podman-compose", "-f", compose_file_path, "up", "-d"]
                    
                    # Executa com ambiente limpo e no diretório correto
                    result = self._run_subprocess_safe(cmd, cwd=compose_dir, timeout=120)
                    
                except Exception as e:
                    return {"error": f"Erro ao executar podman-compose: {str(e)}"}
            else:  # docker
                try:
                    cmd = ["docker-compose", "-f", compose_file_path, "up", "-d"]
                    result = self._run_subprocess_safe(cmd, cwd=compose_dir, timeout=120)
                except Exception as e:
                    return {"error": f"Erro ao executar docker-compose: {str(e)}"}

            if result.returncode == 0:
                return {"success": result.stdout}
            else:
                # Se ainda tiver erro de conmon, tenta abordagem alternativa
                if "conmon" in result.stderr and self.container_engine == "podman":
                    return self._try_alternative_podman_approach(compose_file_path, compose_dir)
                return {"error": result.stderr}
                
        except Exception as e:
            return {"error": str(e)}

    def _try_alternative_podman_approach(self, compose_file_path, compose_dir):
        """Tenta abordagem alternativa para contornar problemas do conmon"""
        try:
            print("Tentando abordagem alternativa devido ao erro do conmon...")
            
            # Primeira tentativa: usar podman run direto (se possível extrair do compose)
            # Segunda tentativa: rodar com --runtime=runc se disponível
            cmd = ["podman-compose", "--podman-run-args", "--runtime=runc", 
                   "-f", compose_file_path, "up", "-d"]
            
            result = self._run_subprocess_safe(cmd, cwd=compose_dir, timeout=120)
            
            if result.returncode == 0:
                return {"success": f"Executado com runtime alternativo: {result.stdout}"}
            else:
                # Última tentativa: sugerir execução manual
                return {
                    "error": f"Erro persistente com conmon. Sugestão: execute manualmente no terminal:\n"
                             f"cd '{compose_dir}' && podman-compose -f '{os.path.basename(compose_file_path)}' up -d\n"
                             f"Erro original: {result.stderr}"
                }
                
        except Exception as e:
            return {"error": f"Tentativa alternativa falhou: {str(e)}"}

    def docker_compose_down(self, compose_file_path):
        """Executa o comando 'podman-compose down' com tratamento robusto."""
        # Se estiver em modo demo, simular o comportamento
        if self.container_engine == "demo":
        ...
        try:
            ...
            # Escolhe o comando baseado no engine disponível
            if self.container_engine == "podman":
                cmd = ["podman-compose", "-f", compose_file_path, "down"]
            else:  # docker
                cmd = ["docker-compose", "-f", compose_file_path, "down"]

            result = self._run_subprocess_safe(cmd, cwd=compose_dir, timeout=60)
    ...
    def docker_compose_down_force(self, compose_file_path):
        ...
        try:
            ...
            cmd = [self.container_engine, "compose", "-f", compose_file_path, 
...
# Substitua sua classe atual por esta versão
manager = DockerManager()
result = manager.docker_compose_up("/home/felipe/Desktop/Delivery_QEMULA (correto)/Delivery_QEMULA/QEMULA_Delivery(Cmd)/Linux_Unix/qemula_system/docker-compose.yml")
print(result)
