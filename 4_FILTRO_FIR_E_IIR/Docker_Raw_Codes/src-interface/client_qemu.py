...
import pexpect
import re
#from leitor_dados import reciever_client as rcv
...
NO_CONFIG_RAM = 0x09
NO_CONFIG_ELF = 0x0A
NO_CONFIG_DEBUG = 0x0B
NO_BREAK_POINT = 0x0C
PASS_LIMIT_MEM_HOST = 0x0D
NOT_RECOGNIZED_COMMAND_DGB = 0x0E
NO_EXIST_LINE = 0x0F
NO_EXIST_VAR = 0x10
NO_EXIST_ADDR = 0x11
INVALID_OPTION_DEBUG = 0x12
...
class QemuEmulator:
    ...
    def __init__(self):
        ...
        # Inicializa a classes necessárias
    ...
    def start_emulation(self, body_content):
        ...
        if self.memory_value is None or self.debug_value is None or self.kernel_loaded is False:
            ...
            # Verifica se a memória foi configurada
            if self.memory_value is None:
                return (NO_CONFIG_RAM, None)
            # Verifica se o debug foi configurado
            elif self.debug_value is None:
                return (NO_CONFIG_DEBUG, None)
            # Verifica se o kernel foi carregado
            elif self.kernel_loaded is False:
                return (NO_CONFIG_ELF, None)
        
        command = (
	    f"./qemu-system-sparc -M leon3_generic -m {self.memory_value}M {self.debug_value} -nographic -no-reboot "
	    f"-chardev socket,id=mon1,host=0.0.0.0,port={QEMU_MONITOR_PORT},server=on,wait=on -mon chardev=mon1 -kernel kernel "
	    f"-chardev socket,id=spwserial,host=0.0.0.0,port={QEMU_INST_PORT2},server=on,wait=on "
        f"-serial tcp:0.0.0.0:{QEMU_INST_PORT1},server=on,wait=on"
	    )
    ...
    def config_ram(self, body_content):
        ...
        # Caso a memória seja maior que a memoria do host
        if memory_value_int % 4 == 0 and memory_value_int != 0 and memory_value_int > os.sysconf('SC_PAGE_SIZE') * os.sysconf('SC_PHYS_PAGES') / (1024. ** 2):
            return (PASS_LIMIT_MEM_HOST, None)
    ...
    def continue_execution(self, body_content):
        ...
        # Caso não haja mais break point no código
        except pexpect.EOF:
            # Retorna o erro de que não há mais break point
            return (NO_BREAK_POINT, None)
    ...
    def verify_variable(self, body_content):
        ...
        # Caso o retorno indique que não existe a variável
        if response == None:
            return (NO_EXIST_VAR, None)
...
# Função para ler de um I/O
    ...
    # Função para carregar um endereço de memória
    def load_addr(self, body_content):
        # Confere o estado da máquina
        if self.current_state == MACHINE_STATE_RUN:
            return (INVALID_MACHINE_STATE, None)
        ...
        # Processa o body_content recebido, em valor do endereço
        self.value_addr = body_content[::].hex().zfill(8)
