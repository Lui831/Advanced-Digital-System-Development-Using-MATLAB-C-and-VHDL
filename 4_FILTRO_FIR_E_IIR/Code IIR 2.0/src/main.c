    ...
    while(1){
       ...
       if(apbuart_get_status(oDevices[u32Cont]) & U32_DR_MASK != U32_DR_COMPARE){
		  ...
		  //float smoothedValue = moving_average(floatValue);
			...
			// Declarando o input parameters e output parameters
		  float input_params[INPUT_PARAMS_SIZE] = {0.2, 0.1, 0.05, 0.03, 0.01}; // Exemplo de parâmetros para a parte feedforward
		  float output_params[OUTPUT_PARAMS_SIZE] = {1, 0.5, -0.3, 0.1, 0.05}; // Exemplo de parâmetros para a parte feedback
		  // Calcula a saída do filtro IIR
			float smoothedValue = iir_filter(input_params, output_params, floatValue);
